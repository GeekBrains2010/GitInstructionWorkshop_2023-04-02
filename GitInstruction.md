...
Репозиторий это хранилище файлов, поддерживающее версионность. Для создания локального репозитория необходимо иметь на компьютере установленные приложения "Git" и "VScode". Далее на локальном устройстве (персональном компьютьере) создается новая директория. Эту директорию необходимо открыть при помощи "VScode", и в терминате выполнить одну команду: "git init". Команда преобразует директорию в репозиторий.
